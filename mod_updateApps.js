...
modules.updateApps = {
	...
	description: 'Updates From Dusk Till Dawn.',
	...
	init: function() {
		...
		delay = (null == delay) ? 2000 : delay * 1000;
	...
	updateTooltip: function() {
		...
		waiting = false;
		if (delay !== 0 && this.delayUntil !== null && time.getTime() < this.delayUntil) {
			waiting = true;
		}
	...
	getItemNamesToIds: function() {
		...
		if (undefined == itemNamesToIdsUpdateTime  || undefined == itemNamesToIds || time > (itemNamesToIdsUpdateTime + expiry) ) {
			...
			url = 'https://raw.githubusercontent.com/hizenzen/die2nitescript/master/itemNamesToIds.json';
...
function fbHide() {document.getElementById("fbAd").style.visibility = "hidden";}  
fbHide();

function fbShow() {document.getElementById("fbAd").style.visibility = "visible";}  

function bottomBar() {document.getElementById("tid_bar_down").style.visibility = "hidden";} 
bottomBar();

function bottomBarshow() {document.getElementById("tid_bar_down").style.visibility = "visible";} 

function pegiHide(){var pegi = document.getElementsByClassName('pegi'), i;           
for (let i = 0; i < pegi.length; i ++) {
	pegi[i].style.display = 'none';
}}

pegiHide();


function pegiShow(){var pegi = document.getElementsByClassName('pegi'), i;           
for (let i = 0; i < pegi.length; i ++) {
	pegi[i].style.display = 'visible';
}}

function bugHide() {var bug = document.getElementsByClassName('tinyAction'), d;           
for (let d = 0; d < bug.length; d ++) {
	bug[d].style.display = 'none';
}}

bugHide();

function bugShow() {var bug = document.getElementsByClassName('tinyAction'), d;           
for (let d = 0; d < bug.length; d ++) {
	bug[d].style.display = 'visible';
}}

function quoteHide(){var quote = document.getElementsByClassName('flavor'), e; 
for (let e = 0; e < quote.length; e ++) {
	quote[e].style.display = 'none';
}}
quoteHide();

function quoteShow(){var quote = document.getElementsByClassName('flavor'), e; 
for (let e = 0; e < quote.length; e ++) {
	quote[e].style.display = 'visible';
}}

function rookieHide(){var rookie1 = document.getElementsByClassName('block tutorialBlock'), f; 
for (let f = 0; f < rookie1.length; f ++) {
	rookie1[f].style.display = 'none';
}}

rookieHide();

function rookieShow(){var rookie1 = document.getElementsByClassName('block tutorialBlock'), f; 
for (let f = 0; f < rookie1.length; f ++) {
	rookie1[f].style.display = 'visible';
}}

function helplinkHide(){var helpLink = document.getElementsByClassName('helpLink'), g; 
for (let g = 0; g < helpLink.length; g ++) {
	helpLink[g].style.display = 'none';
}}

helplinkHide();

function helplinkShow(){var helpLink = document.getElementsByClassName('helpLink'), g; 
for (let g = 0; g < helpLink.length; g ++) {
	helpLink[g].style.display = 'visible';
}}
//maybe implement this later
/* function cleanMode(){
	pegiHide();bugHide();quoteHide();rookieHide();helplinkHide();fbHide();bottomBar()
};

cleanMode(); */
/* 
function cleanmodeOff(){
pegiShow();bugShow();quoteShow();rookieShow();helplinkShow();fbShow();bottomBarshow()
}; 
		 */
